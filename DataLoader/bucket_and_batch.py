...
class bucket_and_batch:
    ...
    def bucket_and_batch(self, texts, labels, labels2idx, vocab2idx, batch_size):
        ...
        def encode(text, vocab2idx):
            return [vocab2idx.get(word, vocab2idx['<UNK>']) for word in text.split(" ")]

        idx2labels = {v: k for k, v in labels2idx.items()}
        original_texts = copy.deepcopy(texts)

        text_ids = [encode(text, vocab2idx) for text in texts]
        PAD = vocab2idx['<PAD>']
        true_seq_lens = [len(text.split(" ")) for text in texts]
