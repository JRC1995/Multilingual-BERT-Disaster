...
for disaster in val_texts:
    ...
    for time in range(times):
        ...
        checkpoint = T.load(
            "../Model_Backup/{}_{}_{}.pt".format(model_name, language, time))
        ...
        output_dir = Path("output/{}_{}_{}_{}.txt".format(disaster,
                                                          model_name, time, language))
        ...
        for id in idx2labels:
            f = open(output_dir, "a")
            for pred, label, label_mask in zip(batch_predictions, batch_labels, batch_label_masks):
                if label_mask[id] == 1:
                    if pred[id] == label[id]:
                        val = 1
                    else:
